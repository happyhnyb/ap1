...
export default function PredictorClient() {
  ...
  useEffect(() => {
    fetch('/api/predictor/status')
      .then((r) => { setServiceUp(r.ok); if (r.ok) return r.json(); })
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [sumRes, histRes, foreRes] = await Promise.all([
        ...
        fetch(`/api/predictor/forecast?${params}`),  // may 404 gracefully
      ]);
          ...
          {forecast && !forecast.insufficient && (
            <div className="card-elevated" style={{ padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                {forecast.mape !== null && (
                  <span style={{ fontSize: 12, color: 'var(--dim)', marginLeft: 'auto' }}>MAPE: {forecast.mape.toFixed(1)}%</span>
                )}
              </div>
              <div className="grid-3" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {forecast.forecast.slice(0, 6).map((f) => (
                  <div key={f.date} style={{ padding: '10px 12px', background: 'var(--bg3)', borderRadius: 10, border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: 11, color: 'var(--dim)', marginBottom: 4 }}>{new Date(f.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</div>
                ...
                <>
                  {history.length > 0 ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--muted)' }}>
                      {serviceUp === false ? 'Start the mandi service to load live data.' : loading ? 'Loading chart data…' : 'No price history available for this selection.'}
                    </div>
                  )}
                </>
              )}
                  ...
                  {forecast ? (
                    <>
                      {forecast.insufficient ? (
                        <div className="notice notice-gold">{forecast.message || 'Insufficient data for a reliable forecast. Need at least 14 days of price history.'}</div>
                      ) : (
                        <div>
                          <div style={{ marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                            <div><span style={{ color: 'var(--dim)', fontSize: 12 }}>Current price</span><div style={{ fontWeight: 700, fontSize: 22, fontFamily: 'Lora,serif' }}>{fmt(forecast.latestPrice)}</div></div>
                            <div><span style={{ color: 'var(--dim)', fontSize: 12 }}>14-day trend</span><div style={{ fontWeight: 700, fontSize: 22, fontFamily: 'Lora,serif', color: forecast.direction === 'up' ? 'var(--green)' : 'var(--red)' }}>{forecast.direction === 'up' ? '+' : ''}{forecast.trend_pct.toFixed(1)}%</div></div>
                            {forecast.mape !== null && <div><span style={{ color: 'var(--dim)', fontSize: 12 }}>Forecast error (MAPE)</span><div style={{ fontWeight: 700, fontSize: 22, fontFamily: 'Lora,serif' }}>{forecast.mape.toFixed(1)}%</div></div>}
                          </div>
                          <table className="table">
                          ...
                          <p style={{ fontSize: 12, color: 'var(--dim)', marginTop: 14, lineHeight: 1.6 }}>
                            Forecast uses Holt's exponential smoothing with trend on {forecast.dataPoints} historical data points.
                            Confidence bands are ±{forecast.mape !== null ? (forecast.mape * 1.5).toFixed(1) : '10'}%.
                            This is a trend model — not a guarantee. Use alongside fundamental market knowledge.
                          </p>
                        </div>
                      )}
                    </>
                    ...
                    <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--muted)' }}>
                      {serviceUp === false ? 'Start the mandi service.' : 'No forecast data available for this selection.'}
                    </div>
                  )}
                </>
              )}
