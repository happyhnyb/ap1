...
export default async function SubscribePage({
  ...
  const stripeEnabled = env.STRIPE_ENABLED;
  ...
  return (
    <main className="container" style={{ paddingBottom: 80 }}>
      ...
      {params.cancelled && !hasPro && (
        <div className="notice notice-yellow" style={{ maxWidth: 500, margin: '0 auto 24px', textAlign: 'center' }}>
      ...
      {hasPro && (
        <div className="notice notice-green" style={{ maxWidth: 500, margin: '0 auto 32px', textAlign: 'center' }}>
          <p style={{ marginBottom: 12 }}>✓ You have an active KYC Pro subscription.</p>
          {stripeEnabled && <ManageSubscriptionButton />}
        </div>
      )}

      {!session && (
        <div className="notice" style={{ maxWidth: 500, margin: '0 auto 24px', textAlign: 'center' }}>
          ...
          <div key={plan.id} className={`card-elevated plan${plan.featured ? ' featured' : ''}`} style={{ position: 'relative', overflow: 'hidden' }}>
            ...
            <div>
              {session && session.plan === plan.id && session.sub_status === 'active' ? (
                ...
                <Link href="/login" className={`btn ${plan.featured ? 'btn-gold' : 'btn-primary'}`} style={{ display: 'flex', justifyContent: 'center' }}>
                  Log in to subscribe
                </Link>
              ) : (
                <button className="btn" style={{ width: '100%', justifyContent: 'center', opacity: 0.5 }} disabled>
